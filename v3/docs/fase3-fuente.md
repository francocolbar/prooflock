...
| **A-6** | **Dos unidades de calentamiento** (NB, RF) por 16 PINIs + antenas + klystrons. Cada unidad tiene potencia **total, parcial o nula** (`On`, `Reduced`, `Off`, más `Ramping`): una alarma local saca un PINI y la unidad pasa a `Reduced` sin dejar de entregar potencia (R-9); la compensación con otros PINIs no se modela y una unidad reducida no vuelve a `On` en el pulso (conservador). Revisión 2026-09-21 (F2); hasta entonces el modelo inhibía la unidad entera. | conservadora en "no vuelve a On"; neutral en el resto | P9, F2a, F2d |
| **A-7** | **Protección local = reducción por el resto del pulso.** R-8 habla de "dynamically controlled limits [...] with fine granularity" (un límite que se recalcula, no un bit que se pega); un des-reducir por hotspot no está publicado, así que la unidad queda en `Reduced` hasta el fin de pulso. Hasta 2026-09-21 el modelo inhibía la unidad entera (política más restrictiva que JET y contraria al concepto de R-9): F2. | conservadora | F2d sobra; nada se pierde |
...
| **A-9** | **Efecto de las paradas sobre el calentamiento.** Unidad con cuatro valores `Off | Ramping | Reduced | On` (A-26). Al entrar a RTPS o a JTT, toda unidad `On` o `Reduced` pasa a `Ramping` (las referencias están sobreescritas / el programa saltó a las formas de onda de terminación, que bajan la potencia: R-3, R-4) y `Ramping` nunca vuelve a `On`. Al entrar a Termination por `Advance` (fin natural), lo mismo. Al llegar a PTN por cualquier camino, `On`/`Reduced`/`Ramping` → `Off` en el mismo paso (R-0, R-11). Con una parada en curso, ninguna orden nueva enciende nada. **Revisión 2 tenía "JTT des-energiza"**: contradecía R-4 (JTT *rampa*) y hacía al JTT más duro que el RTPS stop contra A-1. | conservadora en "no enciende"; neutral en "rampa" | I1, I2, P3, P4 |
...
| **A-13** | **Falla de comunicación y alarmas ciegas** (R-13) son dos caminos: `CommFault{dms, en}` → PTN desde cualquier estado **si el chequeo está habilitado** (`en`, la máscara de la instancia: A-21) e identidad si no (F3); una alarma ciega es el disparador `Blind` de la tabla (A-25). Hasta 2026-09-21 ambos se fundían en un `CommFault` incondicional. | neutral | P12, F3b |
...
| **A-15** | **Sin tiempo real ni vivacidad no acotada.** Ticks abstractos; no se prueba que el pulso termine. Que el watchdog enclave el PTN en `hb_max` ticks y que un DMS armado dispare en `ack_max` ticks sí se prueba (`LAWS_JETPROT_LIVE.bend`, 2026-09-21), en ticks, no en ms. Los números que faltan: VTM 10 ms [S4]; VTM→RTPS 100 Hz, RTPS 500 Hz [S1]; PETRA 2 ms [S7]; NBI off 2 ms, RF off 38 ms, pedido→inyección 50 ms, vuelo del gas 3,4 ms [S6]. **I5 acota un contador, no una latencia.** | — | — |
...
| **A-21** | **Bypass modelado solo para los dos chequeos de fiabilidad** de R-13 (falla de comunicación, alarmas ciegas), como máscara de la instancia (`Mask{comm, blind}`: instancias 1 y 2 con ambos habilitados, instancia 3 con ambos deshabilitados y certificada igual). Las entradas y salidas del PTN (R-10) y la ventana mal configurada siguen fuera: exactamente donde JET perdió 5 + 4 disrupciones (R-15). Es el argumento *a favor* del método: certificar la configuración de cada pulso. Revisión 2026-09-21 (F3). | no conservadora en lo que queda fuera | F3b, `mask_*`, `blind_masked_no_response` |
| **A-22** | ~~Umbral de corriente del DMV fuera del alfabeto~~ **Retirada 2026-09-21: R-14 modelado.** `ip` es estado (el veredicto `Ip ≥ umbral`, entrada `Ip{ok}` como `Plasma{ok}`) y gatea el **armado** del DMS: por debajo del umbral ningún evento arma (IP1); el umbral se evalúa al armar, una caída posterior no desarma (conservador respecto de [S6]). 7 de las disrupciones perdidas de R-15 son de este tipo. | neutral | IP1–IP4, P16, P17 |
...
| **A-24** | **Dos vistas del tiempo** ([S2] §3.4). `prog` es la fase de programa de Level-1: la que indexa la Tabla 1 y que solo `Advance` (y `Reset`) mueve. El JTT no la toca: enciende `jtt`, y la fase de onda es `wave = jtt ? Termination : prog`. Leen `wave`: el permisivo de calentamiento, la ventana del DMV, la guarda del fin de pulso, I2/I3. Leen `prog`: la tabla, `Advance`, D11/D12. Hasta 2026-09-21 el JTT escribía `Termination` en la única fase y una alarma posterior leía la fila Termination (F1). | neutral | F1a–F1d, E3, D12 |
| **A-25** | **Una alarma ciega es un disparador de la tabla** (`Blind`: fila PTN en toda fase para las instancias 1 y 2; sin respuesta cuando la máscara la deshabilita). R-13 dice que el VTM define "blind stop alarms"; su fila no está publicada. | fabricada (la fila) | `asm_*_Blind`, `blind_masked_no_response` |
| **A-26** | **Sin `Inhibited`.** Con la protección local reduciendo (A-7), ningún evento del pulso producía el estado inhibido; una unidad deshabilitada antes del pulso (R-10) es configuración, no evento. Se retiró el valor y con él P8 (`inhibit_latched`) y E1 (`inhibit_source`), que habrían quedado vacuas (gate C2). | — | — |
...
**Revisión 2026-09-21 (bloqueante 4):** 7 fases de programa × 2 (`jtt`) × 4 niveles × 3 DMS × 2 (`plasma_ok`) × 2 (`ip`) × 4²
unidades = **10 752 estados**; alfabeto abstracto de **28 variantes** (`Ip{ok}` ×2, `CommFault{dms, en}` ×4); columnas
23 + 5 × 4 = 43 → **462 336 celdas por orden**. Certificado en runtime (JS): 27 s los dos órdenes; capa concreta 3
instancias × 7 × 2 × 22 = 924 celdas.
