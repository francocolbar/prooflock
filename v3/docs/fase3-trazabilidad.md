...
`fase3-diseno.md` (leyes). Artefactos: `LAWS_JETPROT.bend` (75 leyes desde el 2026-09-21; eran 64), `LAWS_JETPROT_CONF.bend` (137; eran 117),
`LAWS_JETPROT_LIVE.bend` (2 teoremas de respuesta acotada, b5),
...
| SR-4 | Ninguna unidad **shall** energizarse fuera de la ventana, sin plasma, o con una parada en curso; y **shall not** cambiar de estado sin un comando, una pérdida de plasma, el programa, el fin de pulso o una parada que cambie la respuesta | H-E, H-L | R-12 | A-5, A-8 | cláusula I1; `heat_permissive`; `stop_overrides_heat`; `heat_frame`; **`heaton_is_local`**; **`f4_units_frame`** (b4) | preservación; reflexión | — | — |
...
| SR-6 | Una parada marcada para DMS que llega al PTN **shall** armarlo; la espera **shall** estar acotada; una alarma repetida **shall not** reiniciarla | H-C2 | R-11 | A-10, A-11 | `dms_armed_on_demand`; cláusula I5; `tack_frame`; `dms_monotone`; **`ack_timeout_fires`**, **`ack_counts`**, **`heatack_exact`**, **`tack_inc_frame`**, **`tack_reset_frame`**; **`dms_responds`** (b5: armado ⇒ disparado en ≤ `ack_max` ticks sin `Reset`) | reflexión; `cnt_go`; inducción sobre la traza (`PROOF_JETPROT_LIVE_CORE`) | bug6 | `repeated_alarm_restarts_ack` |
| SR-7 | `hb_max` ciclos sin heartbeat **shall** producir PTN | H-D | R-13 | A-12 | cláusula I6; **`watchdog_latches`**, **`hb_counts`**, **`hb_tick_exact`**, **`heartbeat_resets_hb`**, **`hb_frame`**; **`watchdog_responds`** (b5: `hb_max` ticks sin `Heartbeat` ni `Reset` ⇒ PTN, desde cualquier estado del invariante) | `cnt_go`; alcanzabilidad concreta (C5); inducción sobre la traza (`PROOF_JETPROT_LIVE_CORE`) | — | `watchdog_off_by_one` |
...
| SR-9 | Una alarma local **shall** llevar su unidad de potencia plena a potencia **parcial** (un PINI fuera, R-9) y nada más; la reducción **shall** durar el pulso (una unidad reducida **shall not** volver a potencia plena) | H-A2 | R-8, R-9 | A-6, A-7 | `local_is_local` (b4); **`f2a_local_reduces`**, **`f2d_reduced_never_returns`** | reflexión | — | `plasma_ok_restores_reduced` |
| SR-16 | Una alarma concreta **shall** leer la Tabla 1 en la fase de programa de Level-1; un JTT **shall** conmutar la forma de onda y **shall not** mover la fase de programa | H-A1, H-F | R-1, R-4, [S2] §3.4 | A-24 | **`f1a_table_reads_prog`**, **`f1b_stop_keeps_prog`**, **`f1c_wave_ahead`**, **`f1d_wave_frame`**, **`f1e_jtt_exact`**, `stop_phase_exact` (b4), `phase_frame` (b4) | por casos (F1a, F1c); reflexión | — | `jtt_moves_program_phase` |
| SR-17 | Una falla de comunicación **shall** producir PTN cuando la instancia habilita el chequeo y **shall not** hacer nada cuando lo deshabilita; una alarma ciega **shall** pasar por la tabla | H-D, H-H | R-13, [S1] p.1296 | A-13, A-21, A-25 | `commfault_ptn` (b4), **`f3b_commfault_masked_is_noop`**, `mask_inst1/2_checks_on`, `mask_inst3_checks_off`, `asm_*_Blind`, `blind_masked_no_response` | reflexión; `{==}` | — | tercera instancia en `prod/` |
| SR-18 | El DMS **shall not** armarse con la corriente de plasma por debajo del umbral del DMV, y **shall** armarse ante una demanda por encima de él | H-C2 | R-14, [S6] | (A-22 retirada) | **`ip1_low_never_arms`**, **`ip2_arms_on_demand`**, **`ip3_ip_is_input`**, **`ip4_ip_frame`**, `dms_armed_on_demand` (b4), `dms_frame` (b4) | reflexión; P16 | — | `arms_below_threshold` |
...
(R-11 parcial → A-11); el bypass de las entradas y salidas del PTN y las ventanas mal configuradas (R-15 → A-21; las
máscaras de los dos chequeos de fiabilidad sí se modelan desde el 2026-09-21); todo plazo (A-15); la independencia
entre capas (A-17). El umbral de corriente del DMV (R-14) dejó de estar fuera (A-22 retirada; SR-18).
...
| R-8, R-9 | requisito | `Local`, `Reduced`, `local_is_local`, `f2a_local_reduces`, `f2d_reduced_never_returns` | cubierto: una alarma local reduce (un PINI fuera) y no apaga (A-6, A-7, revisión 2026-09-21); la compensación con otros PINIs no se modela |
...
| R-13 | requisito | `CommFault{dms, en}`, `Blind`, las máscaras, el watchdog y sus cinco leyes | cubierto (A-12, A-13, A-21, A-25): los dos chequeos pasan por la máscara de la instancia |
| R-14 | requisito | `dms_window` (fase de onda), `ip`, IP1–IP4 | cubierto desde el 2026-09-21: el umbral de corriente gatea el armado (A-22 retirada) |
...
| A-6 | `local_is_local`, `f2a_local_reduces`, `f2d_reduced_never_returns` | Una unidad reducida no vuelve a potencia plena (conservador); la compensación con otros PINIs no se expresa. |
| A-7 | `f2d_reduced_never_returns` | Política más restrictiva que la de JET en el des-reducir, declarada. |
| A-13, A-21, A-25 | `commfault_ptn`, `f3b_commfault_masked_is_noop`, `mask_*`, `asm_*_Blind`, `blind_masked_no_response` | La máscara es configuración certificada por instancia; la fila `Blind` es supuesta. |
| A-24 | `f1a_table_reads_prog`, `f1b_stop_keeps_prog`, `f1c_wave_ahead`, `f1d_wave_frame`, `stop_phase_exact`, `phase_frame`, `heat_permissive`, I2, I3 | Si la tabla se indexara por la fase de onda (la lectura anterior), un `Slow` tras un JTT escalaría a PTN. |
