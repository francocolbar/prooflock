"""run.py - Phase 2 driver: gates + differential testing of the Bend golden model
...
PROOFS = ["seq/PROOF_SEQ.bend", "seq/PROOF_SEQ_FINITE.bend"]
MAX_EXAMPLES = 3000
...
def main():
    ...
    code, out, err = bend("tests/laws_seq_smoke.bend")
    passed = code == 0 and "ok" in out and not any(ln.startswith("FALSE") for ln in out.splitlines())
    results["gates"]["tests/laws_seq_smoke.bend"] = "all ok" if passed else out[-300:]
    ok_all &= passed
    print(f"[gate] tests/laws_seq_smoke.bend    -> {results['gates']['tests/laws_seq_smoke.bend']}")
