...
P1..P21 / D1..D18 / E1..E11 and the state corollaries as (hypothesis, conclusion) pairs, the
C1 / C3 / V1 conformance checks and `check_cell`. Split out of jetprot_ref.py (blocker 2 of
docs/STATUS_2026-09-21.md) so that the laws read the specification constants
(spec_consts.py) and never the model ones: a mutant of jetprot_ref.py that changes HB_MAX, the
initial state, the heating window or the urgency order is now judged against the spec.
...
is documented; D10 and E8 spell the guard out). `ramp` and `deenergize` are the spec's own.
...
def deenergize(u):
    """what the spec expects a de-energize to do: full power or ramp-down goes to Off, an inhibit stays"""
    return "Off" if u in ("On", "Ramping") else u


def ramp(u):
    """what the spec expects a ramp-down to do: full power goes to Ramping, nothing else moves"""
...
def inv_all(st):
    s, hb, tack = st
    return inv_fin(s) and (s[2] != "DmsArmed" or tack < C.ACK_MAX) and (s[1] == "LPtn" or hb < C.HB_MAX)
...
LAWS = {
    # name: (hypothesis, conclusion); each takes (o, s, e, bt, bh, s2) with s2 = R.step_fin(o, s, e, bt, bh)
    "pres": (lambda o, s, e, bt, bh, s2: inv_fin(s), lambda o, s, e, bt, bh, s2: inv_fin(s2)),
    "hb_shape": (lambda o, s, e, bt, bh, s2: s2[1] != "LPtn",
                 lambda o, s, e, bt, bh, s2: {"CReset": True, "CKeep": s[1] != "LPtn", "CInc": bh}[R.upd_hb(s, e, bh)]),
    "tack_shape": (lambda o, s, e, bt, bh, s2: s2[2] == "DmsArmed",
                   lambda o, s, e, bt, bh, s2: {"CReset": True, "CKeep": s[2] == "DmsArmed", "CInc": bt}[R.upd_tack(s, e, bt, bh)]),
    "P1_latched": (lambda o, s, e, bt, bh, s2: e[0] != "Reset", lambda o, s, e, bt, bh, s2: C.RANK[o][s2[1]] >= C.RANK[o][s[1]]),
    ...
    "P14_phase_monotone": (lambda o, s, e, bt, bh, s2: e[0] != "Reset",
                           lambda o, s, e, bt, bh, s2: C.PHASES.index(s2[0]) >= C.PHASES.index(s[0])),
    "P15_dms_monotone": (lambda o, s, e, bt, bh, s2: e[0] != "Reset",
                         lambda o, s, e, bt, bh, s2: C.DMSS.index(s2[2]) >= C.DMSS.index(s[2])),
...
DEMAND_LAWS = {
    ...
    "D2_soft_stop_ramps": (lambda o, s, e, bt, bh, s2: e[0] == "Stop" and e[1] in ("LJtt", "LRtps")
                           and C.RANK[o][e[1]] > C.RANK[o][s[1]] and "On" in (s[4], s[5]),
...
EXTRA_LAWS = {
    ...
    "E3_stop_phase_exact": (lambda o, s, e, bt, bh, s2: e[0] == "Stop",
                            lambda o, s, e, bt, bh, s2: s2[0] == ("Termination" if (e[1] == "LJtt" and C.RANK[o][e[1]] > C.RANK[o][s[1]]) else s[0])),
    ...
    "E6_hb_tick_exact": (lambda o, s, e, bt, bh, s2: e[0] == "Tick",
                         lambda o, s, e, bt, bh, s2: R.upd_hb(s, e, bh) == ("CInc" if (bh and s[1] != "LPtn") else "CKeep")),
...
def concretize_conforms(limit=20):
    ...
    Returns the offending cells, at most `limit` (the callers only need the first)."""
    bad = []
    for i in (1, 2):
        for p in C.PHASES:
            for c in R.CEVENTS:
                if c[0] == "XAlarm":
                    want = ("Stop", C.TABLE[i][c[1]][C.PHASES.index(p)], c[1] in C.DMS_TRIG and p in C.DMS_WINDOW)
                elif c[0] == "XCommFault":
        ...
        for s in R.all_states():
            for hb in range(C.HB_MAX + 2):
                for tack in range(C.ACK_MAX + 2):
                    st = (s, hb, tack)
                    for c in R.CEVENTS:
                        want = R.step_st(C.ORD_OF_INSTANCE[i], st, R.concretize(i, s[0], c))
